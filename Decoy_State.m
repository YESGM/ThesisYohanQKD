...
for j= 1:4
    ...
    subplot(2,2,i);
        ...
        pdc= 10^(-5); %Dark counts
            ...
            %-----------SKR for attenuated lasers with decoy state (nu,mu(optimized),0)------------
            clear mu
            syms mu 
        ...
        %-----------SKR for attenuated lasers with non-decoy state-----------------
            ...
            lgd= legend('Decoy State optimal ','Non Decoy State \mu = 0.1', 'Single-photon source');
...
for j= 1:4
    ...
    subplot(2,2,i);
        ...
        %-----------SKR for attenuated lasers with non-decoy state-----------------
            ...
            xlabel("Fiber Length [Km]")
            ylabel("Key rate per pulse [bit/pulse]")
            lgd= legend('Decoy State optimal \mu','Non Decoy State \mu = 0.1', 'Single-photon source');
            fontsize(lgd,9,'points')
            title(sprintf('%g%% misalignment',new_index)) 
            sgtitle('SKR VS Distance - SPAD') 
...
hold off
            ...
            xlabel("Fiber Length [Km]")
            ylabel("Key rate per pulse [bit/pulse]")
            title('SSPD - SKR VS Distance Optimal \mu') 
%-------------------------------POISSON---------------------------------------------
            mu=0.1
            r=0:10
            Pois= (mu.^(r)./factorial(r)).*exp(-mu)
            semilogy(r,Pois)
            title('Probability - Number of photons (Poisson distribution \mu=0.1)')
